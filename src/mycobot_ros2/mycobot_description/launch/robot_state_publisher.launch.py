...
def process_ros2_controllers_config(context):
    ...
    install_config_path = os.path.join(
        get_package_share_directory('mycobot_moveit_config'), 'config', robot_name)
    template_path = os.path.join(install_config_path, 'ros2_controllers_template.yaml')
    with open(template_path, 'r', encoding='utf-8') as f:
        content = f.read()
    content = content.replace('${prefix}', prefix).replace('${flange_link}', flange_link)
    os.makedirs(install_config_path, exist_ok=True)
    output_path = os.path.join(install_config_path, 'ros2_controllers.yaml')
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(content)
    real_template = os.path.realpath(template_path)
    src_config_path = os.path.dirname(real_template)
    if src_config_path != install_config_path:
        os.makedirs(src_config_path, exist_ok=True)
        with open(os.path.join(src_config_path, 'ros2_controllers.yaml'), 'w', encoding='utf-8') as f:
            f.write(content)
    return []
...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('use_mock_hardware', default_value='false',
                          choices=['true', 'false'],
                          description='Use mock hardware instead of real robot or Gazebo'),
]
